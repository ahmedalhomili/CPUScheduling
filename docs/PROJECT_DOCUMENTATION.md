...
- **واجهة رسومية** باستخدام Python/Flet (واجهة فقط - الخوارزميات تعمل عبر C++)
- **تكامل C++ ↔ Python** عبر subprocess + JSON API

### 1.2 المتطلبات التقنية

| المتطلب | الإصدار | ملاحظة |
|---------|---------|--------|
| C++ Standard | C++17 | Procedural - بدون OOP |
| Compiler | GCC 13.1+ (MinGW) | مضمّن مع CLion |
| Python | 3.8+ | للواجهة الرسومية فقط |
| Flet | 0.80.5 | GUI Framework |
| CMake | 3.20+ | اختياري - يمكن البناء بـ g++ مباشرة |
...
```
main.cpp                                  (Interactive + CLI + --json API mode)
    └── src/api.cpp                       (api_get_result_json, api_run_algorithm)
            └── src/io_handler.cpp         (print_menu, print_gantt, file I/O)
...
### 2.3 التكامل Python ↔ C++ (JSON Bridge)

```
Python GUI (Flet 0.80.5)                C++ Backend (sched2.exe)
┌───────────────────┐                  ┌────────────────────────┐
│ scheduler_gui.py  │  subprocess      │                        │
│                   │ ──────────────►  │  sched2.exe            │
│  call_cpp_        │  stdin:          │  --json --algo N       │
│  scheduler()      │  "3\n0 7 2\n"   │  --json --all          │
│                   │                  │  --quantum Q           │
│  call_cpp_all()   │  stdout: JSON    │                        │
│                   │ ◄──────────────  │  api_get_result_json() │
└───────────────────┘                  └────────────────────────┘

Stdin Format:                    JSON Output:
count                            {
 arrival burst priority             "algorithm": "...",
 arrival burst priority             "avg_waiting_time": N,
 ...                                "avg_turnaround_time": N,
                                    "cpu_utilization": N,
                                    "processes": [...],
                                    "timeline": [{"process_id": N,
                                                   "start": N,
                                                   "end": N}, ...]
                                 }
```

**ملاحظة مهمة**: Python لا تحتوي على أي تنفيذ للخوارزميات - جميع الحسابات تتم في C++.
...
### 6.2 Sequence Diagram - الوضع التفاعلي (Terminal)
...
### 6.2.1 Sequence Diagram - وضع JSON (GUI Integration)

```
┌──────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────┐    ┌──────────┐
│ User │    │ Python GUI   │    │  sched2.exe  │    │   API    │    │Algorithm │
└──┬───┘    └──────┬───────┘    └──────┬───────┘    └────┬─────┘    └────┬─────┘
   │               │                   │                 │               │
   │ Run Algorithm │                   │                 │               │
   │──────────────>│                   │                 │               │
...
#### البناء بـ g++ (MinGW)
```bash
# إضافة MinGW إلى PATH (PowerShell)
...
./sched2.exe --file data/sample_input.txt --all --quantum 2
./sched2.exe --help
```

#### وضع JSON API (للواجهة الرسومية)
```bash
# خوارزمية واحدة
...
**ملاحظة**: الواجهة تحتاج `sched2.exe` في مجلد المشروع الرئيسي. تظهر حالة الاتصال "C++ Backend Connected" في أعلى يمين الشاشة.
...
- **IDE**: CLion 2025.2.4 / VS Code
- **Compiler**: GCC 13.1.0 (MinGW - مضمّن مع CLion)
- **Build System**: CMake 3.20+ / g++ مباشر
- **GUI Framework**: Flet 0.80.5 (Python)
- **Integration**: subprocess + JSON API
- **Version Control**: Git
- **Standard**: C++17 (Procedural - بدون OOP أو STL لبنى البيانات)
